...
import { ALERT_FORMAT_BG, ALERT_FORMAT_BTN, ALERT_FORMAT_TEXT } from "@/constant";

import { objectValueExtractor } from "@/utils";
...
const Notify = ({ className, handleDissmiss }: INotify) => {
  ...
  ${!stateOpenNotify ? "translate-x-[110%] opacity-0" : "translate-x-0 opacity-100"} 
    flex items-center p-4 rounded-lg w-max max-w-[85vw] md:max-w-[60%] transition-all duration-200 ease-linear fixed z-20 top-[86px] right-[20px]`}
      role="alert"
    >
      <svg
        aria-hidden="true"
